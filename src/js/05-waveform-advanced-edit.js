...
function draw() {
  ...
  if (analyserNode) {
    ...
    // Damp each bass boost so it smoothly boosts from one frame to the next
    for (let i = 0; i < analyserData.length; i++) {
      let signal = analyserData[i];
      if (signal > 0.1) {
        boost = damp(boost, 8, 0.01, deltaTime);
      } else if (signal > 0.08) {
        boost = damp(boost, 8, 0.01, deltaTime);
      } else if (signal > 0.05) {
        boost = damp(boost, 8, 0.01, deltaTime);
    ...
    for (let i = 0; i < analyserData.length; i++) {
      ...
      const amplitude = height * boost;

      const y = map(
        signal,
        -1,
        1,
        height / 2 - amplitude / 4,
        height / 2 + amplitude / 4
      );
